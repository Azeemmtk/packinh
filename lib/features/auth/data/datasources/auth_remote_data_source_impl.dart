...
class AuthRemoteDataSourceImpl implements AuthRemoteDataSource {
  ...
  Future<UserModel> signInWithGoogle() async {
    try {
      ...
      if (existingUser == null) {
        ...
        if (existingUser.role != 'hostel_owner') {
  ...
  Future<void> saveUserToFirestore(UserModel user) async {
    try {
      await firestore.collection('hostel_owners').doc(user.uid).set(user.toFirestore());
    } catch (e) {
  ...
  Future<UserModel> updateUserInFirestore(UserModel user) async {
    try {
      ...
      await firestore.collection('hostel_owners').doc(user.uid).update(updateData);
  ...
  Future<UserModel> signInWithEmail(String email, String password) async {
    try {
      ...
      final userFromFirestore = await getUserFromFirestore(firebaseUser.uid);
      if (userFromFirestore != null) {
        if (userFromFirestore.role != 'hostel_owner') {
